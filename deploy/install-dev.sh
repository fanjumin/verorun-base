...
# ── systemd services — identical to install-local.sh ──────────────────
...
# VeroGuard Guardian environment config — generated by install-dev.sh
...
# Managed by VeroRun install-dev.sh — regenerated on every install/update
# Grants ${APP_USER} passwordless one-click update for VeroRun services
${APP_USER} ALL=(root) NOPASSWD: /bin/bash ${APP_HOME}/deploy/install-dev.sh update
${APP_USER} ALL=(root) NOPASSWD: /bin/bash ${APP_HOME}/deploy/install-dev.sh restart
...
do_seed() {
    ...
    if [ ! -f "${VENV_DIR}/bin/python" ]; then
        ...
        echo -e "${INFO} Run 'install-dev.sh' first"
        exit 1
    fi
