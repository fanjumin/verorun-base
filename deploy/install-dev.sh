...
# 审计 C-1：部署模式（dev）——统一函数（lib/common.sh）据此区分行为，必须在 source 前定义
DEPLOY_TYPE="dev"
...
# ── .env generation — 审计 C-1：generate_env 已统一至 lib/common.sh（DEPLOY_TYPE=dev 驱动，无插件 LAN 模式） ──

# ── Nginx — 审计 C-1：write_nginx_config 已统一至 lib/common.sh（DEPLOY_TYPE=dev 驱动，无域名 default_server 模板） ──

# ── Fresh install — 审计 C-1：do_install 已统一至 lib/common.sh（DEPLOY_TYPE=dev 驱动，排除插件） ──

# ── Summary — 审计 C-1：print_summary 已统一至 lib/common.sh（DEPLOY_TYPE=dev 驱动，显示 NOT installed） ──

# ── Incremental update — 审计 C-1：do_update 已统一至 lib/common.sh（DEPLOY_TYPE=dev 驱动） ──
