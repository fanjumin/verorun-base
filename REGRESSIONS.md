...
## 2026-07-12 B — 频道管理 (channels) 点击报 `Cannot set properties of null (setting 'innerHTML')`

### 问题现象
后台点"频道管理"→切换频道（飞书/企微/QQ 等）时，浏览器控制台报错：
```
Uncaught (in promise) TypeError: Cannot set properties of null (setting 'innerHTML')
    at admin:1414:49
```
接口正常返回，但页面渲染失败，部分频道标签页空白。

### 根本原因
代码位置：[admin_imgateway.html:64-159](file:///f:/Sites/VeroRun/plugins/im_gateway/templates/admin_imgateway.html#L64-L159)

`_chLoadGenericTab()` 和 `_wcLoadTab()` 在所有 `document.getElementById('ch-body').innerHTML = ...` 处**没有判空**。当用户快速切换菜单或频道时，`#mc` 被其他菜单的 `innerHTML` 覆盖，`#ch-body` 已从 DOM 移除，但异步 `fetch` 回调仍在执行，`getElementById` 返回 `null` 直接 `.innerHTML` 导致 TypeError。

### 修复措施
新增 `_chBody()` 辅助函数，所有 8 处 `_chBody().innerHTML` 调用全部加判空保护：

| 位置 | 保护方式 |
|------|----------|
| `_chLoadGenericTab` 函数入口 | `var b=_chBody(); if(!b)return;` — 早期退出 |
| `.then` 错误分支 | `var b=_chBody(); if(b)b.innerHTML=...` |
| `.then` 成功分支 | `var b=_chBody(); if(b)b.innerHTML=...` |
| `.catch` 分支 | `var b=_chBody(); if(b)b.innerHTML=...` |
| `_wcLoadTab` 函数入口 | 同上 |
| `.then` 错误/成功/`.catch` | 同上（共 4 处） |

### 永久防范
1. 任何通过 `getElementById` 获取 DOM 后直接操作 `innerHTML` 的异步回调，**必须判空**——DOM 可能在异步等待期间被其他操作移除
2. 约定：插件内统一用 `_xxxBody()` 辅助函数替代裸 `getElementById`，便于集中判空
3. 切换菜单/标签页时若需取消旧请求，考虑用 AbortController 或标志位，从源头避免竞态
...
## 2026-07-12 全站访问慢（登录/后台/菜单加载延迟）— Tailscale 覆写 /etc/resolv.conf 导致服务端自身域名 DNS 解析奇慢（旧病复发根因）

### 问题现象
- 从统一登录窗口 → 管理员后台 → 后台所有菜单，打开加载都有很长延迟（数秒级）
- 反复出现："解决多次，每次没几天又旧病复发"
- 一度怀疑 CDN

### 诊断过程与决定性证据
分层实测（服务器本机执行 curl），逐层排除，定位到 **DNS 解析层**：

| 访问方式 | dns | total |
|---|---|---|
| 本地直连后端 `127.0.0.1:8084` (带 Host 头) | — | **0.003s** |
| `--resolve` 强制解析到 127.0.0.1（跳过系统 DNS） | 0.00003s | **0.010s** |
| 用域名 `https://easykai.cn/admin/`（走系统 DNS） | **1.5~2.0s** | 1.5s，偶发 **9.3s** |

同一后端、同一 Nginx，唯一差别是**是否走系统 DNS 解析**。后端本身只要 3ms，走 DNS 就慢 500~3000 倍。
...
search bison-kokanue.ts.net
```

### 根本原因
- 服务器装了 **Tailscale**，它接管了 `/etc/resolv.conf`，把 DNS 指向 MagicDNS `100.100.100.100`。
- 解析公网域名（easykai.cn 等自身域名）时，MagicDNS 需转发到上游，耗时 1.5s 起、偶发 9s 超时。
- 应用处理一次完整请求链（登录 → SSO 跨子域跳转 → 后台首页 → 各菜单接口 → 健康检查回调）时，服务端内部会**反复对自身域名发起解析**，每次都吃这个慢 DNS，逐级累加放大为用户感受到的"每个菜单都卡"。
- **"旧病复发"根源**：Tailscale 每次重连/重启都会**重新覆写 `/etc/resolv.conf`**，所以任何针对 resolv.conf 的修复都会被冲掉，几天后复发。

### 修复措施（幂等、持久、零风险）
在 `/etc/hosts` 固定服务端自身域名到本地环回。`/etc/hosts` 优先级高于 `resolv.conf`，且 **Tailscale 不会覆写 hosts**，从此自身域名解析走本地、瞬时完成，彻底免疫 Tailscale DNS 抖动：

```
# 追加到 /etc/hosts 末尾（带 # easykai-local-dns-fix 标记，幂等可复跑）
127.0.0.1 easykai.cn www.easykai.cn platform.easykai.cn agent.easykai.cn # easykai-local-dns-fix
```

同时启用被注释掉的 Nginx gzip（次要优化，减小后台大 JS/CSS 传输体积）：
- `/etc/nginx/nginx.conf` 里 `gzip_vary/gzip_proxied/gzip_comp_level/gzip_buffers/gzip_http_version/gzip_types` 六行原本被 `# ` 注释（行首是 **Tab**），全部取消注释。
- `nginx -t` 通过 → `systemctl reload nginx`。
- 验证：`chart.min.js`(205KB) 与 admin/login HTML 响应头均返回 `Content-Encoding: gzip`。

备份：`/etc/hosts.bak.easykai`、`/etc/nginx/nginx.conf.bak.easykai`。

### 修复效果（服务端本机实测）
| 指标 | 修复前 | 修复后 |
|---|---|---|
| DNS 解析 | 1.5~2.0s | **0.0006s** |
| admin total | 1.5~9.4s | **0.010s** |
| root total | 1.5s | **0.012s** |
| agent 子域 total | 1.5s+ | **0.010s** |

### 永久防范
1. **服务端访问自身域名慢 → 第一件事查 `cat /etc/resolv.conf` 和 `curl -w '%{time_namelookup}'`**，确认是否 DNS 解析层的问题，不要一上来就怀疑 CDN / 后端代码 / Nginx 转发。
2. **凡是装了 Tailscale/VPN 且会接管 resolv.conf 的服务器，自身域名必须写死在 `/etc/hosts`**，不要依赖 resolv.conf（会被覆写，导致"旧病复发"）。
3. **区分"服务端 DNS 慢"与"客户端网络慢"**：`--resolve` 跳过 DNS 若立即变快，即可确诊是本机 DNS 解析问题。
4. `/etc/hosts` 修改块统一带 `# easykai-local-dns-fix` 标记，用 `grep -q` 判断幂等，可安全重复执行。
5. 若日后 Tailscale MagicDNS 恢复正常，可保留此 hosts 记录（自身域名走本地环回本就是更优选择，零副作用）。