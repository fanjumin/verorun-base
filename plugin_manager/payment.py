...
class PaymentRouter:
    ...
    def get_provider(self, channel: str = 'alipay') -> PaymentProvider:
        """获取支付提供方。

        如果支付宝未配置（stub），自动降级为 mock。
        """
        provider = self._providers.get(channel)
        if provider is None:
            provider = self._providers['mock']
        # 如果支付宝未配置但有加密库，也降级
        if channel == 'alipay' and isinstance(provider, AlipayProvider):
            if provider._is_stub:
                print('[Payment] Alipay not configured, falling back to mock')
