        ...
        if payment_trade_no:
            try:
                ...
                if payment_method == 'alipay':
                    from routes.subscription.gateway.alipay import refund_order as _alipay_refund
                    refund_result = _alipay_refund(row['order_id'], amount)
                elif payment_method == 'wechat':
                    from routes.subscription.gateway.wechat import refund_order as _wechat_refund
                    refund_result = _wechat_refund(row['order_id'], amount)
                elif payment_method == 'stripe':
                    from routes.subscription.gateway.stripe import refund_order as _stripe_refund
                    refund_result = _stripe_refund(payment_trade_no, amount)
                elif payment_method == 'paypal':
                    from routes.subscription.gateway.paypal import refund_order as _paypal_refund
