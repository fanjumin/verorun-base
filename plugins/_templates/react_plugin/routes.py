#!/usr/bin/env python3
"""
React Demo Plugin — routes

iframe 页面路由（§15.4）：/admin/react-demo/
...
react_demo_bp = Blueprint(
    'react_demo', __name__,
    url_prefix='/admin/react-demo',
    template_folder='templates',
    static_folder='static',
    static_url_path='/admin/react-demo/static',
)

# 静态资源免鉴权（iframe 加载时静态文件不带 token）
_AUTH_EXEMPT_PATHS = ('/admin/react-demo/static',)


@react_demo_bp.before_request
def _check_auth():
    path = request.path
...
def index():
    ...
    return render_template(
        ...
        sso_token=token,
        translations={
            'demo.title': _('React Plugin Demo'),
            'demo.desc': _('This page is rendered by React 18 (local UMD, no CDN).'),
...
@react_demo_bp.route('/api/hello')
def hello():
    """示例同域 API（前端携带 Authorization: Bearer <token> 访问）"""
    return jsonify({'success': True, 'message': 'Hello from React plugin', 'plugin': 'react_plugin'})
