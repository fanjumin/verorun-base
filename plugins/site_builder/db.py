...
def get_db():
    """返回独立库连接（search_path: site_builder, public）。"""
    conn = get_raw_connection()
    conn.autocommit = False
    try:
        wrapped = SiteBuilderConnection(conn)
        wrapped.execute("SET search_path TO site_builder, public")
        return wrapped
    except Exception:
        conn.close()
        raise
