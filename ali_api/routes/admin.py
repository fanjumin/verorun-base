...
from ali_api.config import config
from ali_api.services.alibaba_client import get_client
from ali_api.services.alibaba_client_v2 import (
    get_access_token, refresh_access_token, get_auth_url,
    get_product as get_product_v2, call_api,
    generate_signature,
)
from ali_api.services.rate_limiter import get_rate_limit_manager
from ali_api.services.cache_service import get_cache_service
from ali_api.services.ai_processor import get_ai_processor, is_ai_available
from ali_api.models import get_db, AliApiItem, AliApiLog, AliApiUserStats

# 创建蓝图
ali_admin_bp = Blueprint('ali_admin', __name__, url_prefix='/admin/ali-api',
...
def _validate_redirect_uri(uri):
    ...
    try:
        ...
        # 不允许带 userinfo 的 URL（防范 user:pass@host 绕过）
...
def _get_valid_token(user_id: int):
    ...
    from ali_api.models import AliApiToken
    with get_db() as conn:
...
def oauth_url():
    ...
    app_key = config['alibaba']['app_key']
    if not app_key:
        return _error('未配置 AppKey')
    
    # 生成随机 state（含 CSRF 防护）+ 持久化
    state = secrets.token_urlsafe(16)
    from ali_api.models import OAuthState
    with get_db() as conn:
...
def v2_get_product(product_id):
    """按商品ID查询 1688 商品详情（新版 API，需要 access_token）"""
    admin = _require_admin()
    if not admin:
        return _error('请先登录', 401)
...
if __name__ == "__main__":
    ...
    for rule in app.url_map.iter_rules():
        if rule.endpoint.startswith('ali_admin.'):
