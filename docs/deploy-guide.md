...
**One-command:**
```bash
curl -sSL https://cdn.jsdelivr.net/gh/fanjumin/verorun-base@master/deploy/install-dev.sh | sudo bash
```
...
| 脚本 | 场景 | 一键命令 |
|------|------|---------|
| `install.sh` | 公网生产部署（需域名） | `curl -fsSL https://cdn.jsdelivr.net/gh/fanjumin/verorun-base@master/deploy/install.sh \| sudo bash -s -- install your-domain.com` |
| `install-local.sh` | 局域网 / 单机无域名 | `curl -sSL https://cdn.jsdelivr.net/gh/fanjumin/verorun-base@master/deploy/install-local.sh \| sudo bash` |
| `install-dev.sh` | 开发工作站（私有库 SSH） | `curl -sSL https://cdn.jsdelivr.net/gh/fanjumin/verorun-base@master/deploy/install-dev.sh \| sudo bash` |
| `install-code.sh` | 团队内网服务器（私有库 SSH + 全量 plugins） | `curl -sSL https://cdn.jsdelivr.net/gh/fanjumin/verorun-base@master/deploy/install-code.sh \| sudo bash` |
