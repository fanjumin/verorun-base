...
侧栏菜单定义在 `GROUPS` 数组中（`admin.html`），共 **11 个分组**，支持折叠展开：

| 分组名称 | 折叠 | 包含模块 |
|---|---|---|
| **仪表盘** | 否 | 仪表盘 |
| **AI 对话** | 否 | 指令控制台 |
| **系统管理** | 是 | 基本设置、OAuth 登录、导航设置、模型维护、管理员设置、品牌设置、站群管理、模板管理 |
| **内容生态** | 是 | 信息捕获、全媒体创作、下载管理、媒体库 |
| **AI 创作** | 是 | PPT 生成、图像生成、多媒体 |
| **运行策略** | 是 | 智能体系、数据清洗、自动调度、IM 网关 |
| **运营支撑** | 是 | 套餐管理、订阅列表、订单管理、收入看板、优惠券、扣款日志、完成度奖励、广告管理、商品分类、商品管理、订单管理、优惠券管理、购买记录 |
| **消息与支持** | 是 | 邮件服务、通知推送、用户工单 |
| **租户生态** | 是 | 主体管理、实例节点、访问令牌 |
| **风控审计** | 是 | 动态审计、评论审核 |
| **运维数据** | 是 | 统计分析、健康巡检、Token 用量、操作日志 |
