...
@admin_bp.route('/generate-ppt', methods=['POST'])
def generate_ppt():
    admin, err = _require_admin()
    if err: return err
    data = request.get_json() or {}
    ...
    # 获取 DeepSeek V4 Flash 用于快速生成
    with get_db() as conn:
        row = conn.execute(
            "SELECT value FROM system_config WHERE key='deepseek_api_key'"
        ).fetchone()
    api_key = row['value'] if row and row['value'] else ''
    if not api_key:
        return jsonify({'success': False, 'error': 'DeepSeek API Key 未配置，请在基本设置中配置'}), 500
...
- 语言与主题一致'''
    ...
    try:
        ...
        filepath = _os_media.path.join(MEDIA_DIR, filename)
        prs.save(filepath)
...
@admin_bp.route('/generate-image', methods=['POST'])
def generate_image():
    """图像生成接口"""
    admin, err = _require_admin()
    if err: return err
    ...
    try:
        # 通过 HTTP 调用 agent_matrix 的 dispatch 执行图像增强
        import requests as _req, json as _json, time as _time, uuid as _uuid, os as _os
        media_dir = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)),
                                   '..', '..', 'data', 'media', 'temp')
        ...
        with open(_os.path.join(media_dir, fn), 'w') as f:
            _json.dump(record, f)
...
@admin_bp.route('/media/download/<filename>')
def media_download(filename):
    """下载生成的媒体文件（PPT、视频等）"""
    admin, err = _require_admin()
    if err: return err
    import os as _dl_os
    dl_dir = _dl_os.path.join(_dl_os.path.dirname(_dl_os.path.abspath(__file__)),
                              '..', '..', 'data', 'media', 'temp')
    _dl_os.makedirs(dl_dir, exist_ok=True)
    from flask import send_file
    filepath = _dl_os.path.join(dl_dir, filename)
    if not _dl_os.path.exists(filepath):
