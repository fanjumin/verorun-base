...
def init_db():
    with get_db() as conn:
        conn.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS brand_settings (
                ...
                software_name   TEXT NOT NULL DEFAULT '易站AI',
            ...
            CREATE TABLE IF NOT EXISTS tm_brand_settings (
                id              INTEGER PRIMARY KEY CHECK (id = 1),
                site_name_cn    TEXT NOT NULL DEFAULT '易站',
    ...
    with get_db() as m:
        ...
        if row['c'] == 0:
            kb_seeds = [
                ...
                ('kb_company_002','公司定位与愿景','易站智能由徐州易开网络科技有限公司研发，定位为AI驱动的智能建站平台。公司愿景是让每个企业都能低成本拥有专业级的网站和小程序，通过AI技术降低建站门槛，助力中小企业数字化转型。','定位,愿景,使命,AI驱动,智能建站,数字化转型,中小企业','company',8),
                ('kb_product_001','易站智能概述','易站智能是一站式智能建站平台，核心功能包括：AI智能体社区（多类型AI助手协作）、AI内容工厂（自动生成文案/图片/SEO）、智能工作流引擎（可视化拖拽搭建）、抖音小程序开发与运营、多端适配（PC+移动+小程序）。','易站智能,AI建站,概述,功能,智能体,内容工厂,工作流,抖音小程序,多端适配','product',10),
                ('kb_product_002','AI智能体社区','AI智能体社区是易站智能的核心模块，内置多种AI助手：SEO优化助手、文案生成助手、图片设计助手、数据分析助手、客服助手等。每个智能体专注于特定任务，可独立工作也可协作完成复杂建站需求。','智能体,AI助手,SEO,文案,图片,数据分析,客服,协作','product',9),
                ...
                ('kb_product_005','抖音小程序开发','易站智能支持一键生成抖音小程序，与抖音生态深度集成。功能包括：抖音账号授权、小程序模板选择、支付集成、直播带货组件、短视频嵌入、数据分析看板等。','抖音,小程序,抖音小程序,直播,带货,支付,短视频,授权','product',9),
                ('kb_product_006','多端适配能力','易站智能支持一次搭建、多端发布：PC网站、移动H5、微信小程序、抖音小程序、支付宝小程序等。自动适配不同终端的屏幕尺寸和交互方式，确保用户体验一致。','多端,适配,PC,H5,微信小程序,支付宝小程序,响应式,跨平台','product',8),
                ('kb_price_001','价格体系概述','易站智能提供灵活的定价方案：基础版适合个人/初创企业（展示型网站），专业版适合中小企业（含AI内容工厂+SEO优化），企业版适合大型企业（含全部功能+定制开发+专属客服）。具体价格请咨询客服获取最新报价。','价格,多少钱,费用,报价,定价,收费,套餐,基础版,专业版,企业版','price',10),
                ...
                ('kb_tech_001','AI技术优势','易站智能采用最新大语言模型技术，结合自研的建站领域知识图谱，实现智能化的网站搭建。AI可理解用户需求描述，自动推荐合适的模板、配色方案和内容布局，大幅缩短建站周期。','AI技术,大模型,知识图谱,智能推荐,模板,配色,布局,技术优势','tech',8),
                ('kb_tech_002','源码交付说明','易站智能支持源码交付，签约后客户可获得完整项目源码。源码包含前端代码、后端接口、数据库脚本等，客户可自行部署和二次开发。源码交付适用于专业版和企业版客户。','源码,交付,代码,部署,二次开发,前端,后端,数据库','tech',9),
                ('kb_tech_003','安全与性能','易站智能采用HTTPS加密传输、数据备份、DDoS防护等安全措施。网站性能方面：CDN加速、图片懒加载、代码压缩、缓存策略等，确保网站加载速度快、运行稳定。','安全,性能,HTTPS,备份,CDN,加速,加载速度,稳定,防护','tech',7),
                ('kb_service_001','合作流程','易站智能合作流程：1.需求沟通（了解您的业务需求和预算）；2.方案定制（AI生成个性化建站方案）；3.合同签订（明确交付内容和时间节点）；4.开发搭建（AI+人工协作高效交付）；5.验收上线（测试通过后正式发布）；6.售后维护（持续技术支持）。','流程,合作,步骤,需求,方案,合同,开发,验收,售后,维护','service',9),
                ('kb_service_002','售后服务','易站智能提供完善的售后服务：免费维护期（基础版1年/专业版2年/企业版3年）、7×24小时技术支持、定期系统更新、紧急故障2小时响应、免费培训（企业版）。维护期后可续费延长。','售后,维护,技术支持,更新,故障,培训,续费,服务','service',8),
                ('kb_service_003','行业解决方案','易站智能针对不同行业提供专属解决方案：电商零售（商品展示+在线支付）、教育培训（课程展示+在线报名）、餐饮美食（菜单展示+外卖对接）、企业服务（品牌展示+线索收集）、房地产（楼盘展示+VR看房）等。','行业,解决方案,电商,教育,餐饮,企业服务,房地产,VR看房,外卖','service',8),
                ('kb_faq_001','建站需要多长时间','使用易站智能，展示型网站最快1天即可上线，企业官网通常3-5个工作日，含小程序的综合方案约7-10个工作日。具体时间取决于需求复杂度和定制化程度。','时间,多久,周期,上线,工作日,快速,几天','faq',9),
                ('kb_faq_002','是否需要技术基础','易站智能采用可视化拖拽操作，无需编程基础即可使用。AI助手会引导您完成每一步操作。如果您有特殊定制需求，我们的技术团队会提供专业支持。','技术基础,编程,代码,不会,简单,操作,难不难,容易','faq',9),
                ('kb_faq_003','是否支持SEO优化','易站智能内置SEO优化功能，AI可自动生成TDK（标题、描述、关键词）、优化页面结构、生成sitemap、配置301重定向等。同时提供SEO分析报告和改进建议。','SEO,优化,搜索引擎,排名,TDK,sitemap,百度,Google','faq',8),
                ('kb_faq_004','域名和服务器说明','易站智能可协助客户完成域名注册和服务器配置。客户可使用自有域名，也可通过我们代购。服务器采用云部署方案，自动扩容，保障网站稳定运行。域名和服务器费用不包含在建站套餐内。','域名,服务器,云部署,扩容,注册,代购,备案','faq',7),
            ]
            for s in kb_seeds:
    ...
    with get_db() as ms:
        ...
        if cnt['c'] < 25:
            faq_seeds_data = [
                ('kb_faq_faq_p1', '你们的产品有哪些功能？', '易站智能(RuiCe AI)是生成式AI平台，支持智能对话（多模型）、AI建站、AI内容生成、数据清洗、知识库管理等。具体可查看官网产品页面或试用体验。', '功能,产品,能力,特性', 'faq', 6),
                ...
                ('kb_faq_whitepaper', '易站智能白皮书', '易站智能(RuiCe AI)是新一代AI驱动的智能建站与企业数字化平台。系统集成了AI聊天机器人、知识库管理（RAG）、数据清洗、内容工厂、CMS门户、订阅计费、多站点管理等完整功能。核心优势：AI原生架构、一体化SSO、开箱即用支付、强大的后台管理和灵活的主题系统。', '白皮书,产品介绍,技术架构,AI建站,功能概述', 'company', 10),
    ...
    # ── Migration: tm_brand_settings site_name_cn → 易站 ──
    with get_db() as m:
        row = m.execute("SELECT site_name_cn FROM tm_brand_settings WHERE id=1").fetchone()
        if row and row["site_name_cn"] == 'TradeMind':
            m.execute("UPDATE tm_brand_settings SET site_name_cn='易站' WHERE id=1")
            m.commit()
            print("[Migration] tm_brand_settings.site_name_cn updated to 易站")
